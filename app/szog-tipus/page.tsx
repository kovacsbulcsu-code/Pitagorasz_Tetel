...
export default function SzogTipus() {
  ...
  const getResult = () => {
    ...
    if (Math.abs(a2b2 - c2) < 0.1) return { msg: "DERÉKSZÖGŰ", color: "text-emerald-400", type: 'right' };
    if (a2b2 > c2) return { msg: "HEGYESSZÖGŰ", color: "text-emerald-400", type: 'acute' };
    return { msg: "TOMPASZÖGŰ", color: "text-emerald-400", type: 'obtuse' };
  };
  ...
  return (
    <main className="min-h-screen bg-[#02040a] text-white flex flex-col items-center justify-center p-6 font-sans relative overflow-hidden">
      ...
      <div className="absolute inset-0 z-0 bg-[radial-gradient(circle_at_50%_50%,#0a2f1f_0%,transparent 70%)] opacity-30"></div>
      ...
      <div className="w-full max-w-5xl relative z-10">
        ...
        <div className="grid md:grid-cols-2 gap-10 items-stretch">
          ...
          {/* Beviteli Panel */}
          <div className="bg-[#0b1120]/70 border border-emerald-500/20 p-8 rounded-[2.5rem] shadow-[0_25px_50px_-12px_rgba(0,0,0,0.7),0_0_15px_rgba(16,185,129,0.05)] backdrop-blur-md flex flex-col justify-between hover:border-emerald-500/30 transition-colors">
            <div>
              <div className="inline-block border border-emerald-500/40 bg-emerald-500/10 px-3 py-1 rounded-md mb-8 shadow-[0_0_15px_rgba(16,185,129,0.2)]">
                 <span className="text-emerald-400 font-black italic text-sm tracking-tighter uppercase">Szögtípus_v1.0</span>
              </div>
            ...
            <div className="mt-10 border border-slate-800/60 bg-[#030712]/60 rounded-2xl p-5 flex items-center justify-between shadow-inner text-center">
               ...
               <span className={`text-[10px] font-mono font-bold ${res.type === 'error' ? 'text-red-500' : 'text-emerald-400'}`}>
                 {res.type === 'empty' ? 'IDLE' : 'COMPUTING_TYPE'}
               </span>
            </div>
          </div>
          ...
          <div className="bg-[#0b1120]/50 border border-emerald-500/10 rounded-[2.5rem] p-8 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.6)] backdrop-blur-md relative flex flex-col items-center justify-between min-h-[500px]">
            ...
            <div className="w-full space-y-4 mt-6">
              <div className="bg-[#030712]/80 border border-slate-800 p-6 rounded-2xl flex justify-between items-center shadow-lg border-b-emerald-500/30">
                 ...
                 <span className={`font-mono text-xl font-black ${res.color} tracking-tighter drop-shadow-[0_0_10px_rgba(16,185,129,0.4)]`}>
                   {res.msg}
                 </span>
              </div>
              <p className="text-[9px] text-emerald-400 font-bold text-center uppercase tracking-[0.4em] opacity-30">
                Angle Classification Engine active
              </p>
            </div>
          </div>
